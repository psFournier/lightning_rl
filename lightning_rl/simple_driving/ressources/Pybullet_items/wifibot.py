...
class Wifibot:
    def __init__(self, client, is_discrete=True, horizontal_res=320, vertical_res=240, fov=45, aspect=1, near_val=0.1,
                 far_val=100):
        self.client = client
        self.f_name = os.path.join(os.path.dirname(__file__), '../URDF/wifibot/wifibot.urdf')

        # Car parameters
        self.car = None
        self.action_list = [[1, 0], [-1, 0], [0, 0.5], [0, -0.5], [0, 0]]
        # 0: forward ; 1: backward ; 2: move left ; 3: move right ; 4: stay still
    ...
    def apply_action(self, action):
        ...
        p.setJointMotorControl2(self.car, 1, p.VELOCITY_CONTROL, targetVelocity=rightWheelVelocity)
        p.setJointMotorControl2(self.car, 2, p.VELOCITY_CONTROL, targetVelocity=leftWheelVelocity)
        p.setJointMotorControl2(self.car, 3, p.VELOCITY_CONTROL, targetVelocity=rightWheelVelocity)
        p.setJointMotorControl2(self.car, 4, p.VELOCITY_CONTROL, targetVelocity=leftWheelVelocity)
